...
# Define your angled region (polygon)
pts = np.array([
    [int(FRAME_WIDTH * 0.4), int(FRAME_HEIGHT * 0.3)],  # A
    [int(FRAME_WIDTH * 0.7), int(FRAME_HEIGHT * 0.2)],  # B
    [int(FRAME_WIDTH * 0.8), int(FRAME_HEIGHT * 0.9)], # D
    [int(FRAME_WIDTH * 0.1), int(FRAME_HEIGHT * 0.9)], # C
], np.int32)

# Make it a shape OpenCV understands
...
def runPipeline(image, llrobot):
    ...
    # Changed from yellow to blue detection
    lower_blue = np.array([90, 100, 50])
    upper_blue = np.array([120, 255, 255])
    mask = cv2.inRange(hsv, lower_blue, upper_blue)

    contours, blue_hierarchy, blue_gray, isDebug, debug_info = process_color(image, mask)
    if isDebug:
